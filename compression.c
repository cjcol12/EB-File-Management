...
unsigned int compressed_array[ROW][25] = {{0}};
...
void compress(){
    ...
    for(int i = 0; i < ROW; i++) {
        ...
        for(int j = 0; j < COL; j++){
            ...
            unsigned char this_element = array2d[i][j];
            unsigned char next_element = (j < COL - 1) ? array2d[i][j+1] : 0;
            unsigned char element_2_away = (j < COL - 2) ? array2d[i][j+2] : 0;
            unsigned char element_3_away = (j < COL - 3) ? array2d[i][j+3] : 0;
